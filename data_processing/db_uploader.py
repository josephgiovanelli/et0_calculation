...
def get_params(sensor_id,valueTypeId,conn):
  if(isinstance(sensor_id, str)):
    ...
    conn.execute("""SELECT * FROM transcoding_sensor WHERE "detectedValueTypeId" = %s AND xx = %s AND yy = %s""",[valueTypeId,x,z,])
  else:
    conn.execute("""SELECT * FROM transcoding_sensor WHERE "detectedValueTypeId" = %s AND xx IS NULL AND yy IS NULL""",[valueTypeId,])
...
def sim_to_db(wc_path,initialState,costMatrix,approach,valueType,valueTypeId,conn):
  ...
  for _, row in dataset.iterrows():
    if row['variable'] in dictionary:
      ...
      try:
        x, z, unit, detectedValueTypeId, detectedValueDescription = get_params(row['variable'],valueTypeId,sql)
        dictionary[row['variable']] = [x,z,unit,detectedValueTypeId,detectedValueDescription]
...
def insert_with_string_io(df: pd.DataFrame,schema,conn):
        ...
        with conn.cursor() as cursor:
            try:
                cursor.copy_expert("COPY smart_irrigation.public."+schema+" FROM STDIN (FORMAT 'csv', HEADER false)", buffer)
                conn.commit()
