...
def pressureFromAltitude(height):
    return P0 * math.pow(
        1.0 + height * LAPSE_RATE_MOIST_AIR / TEMP_P0,
...
def computeHourlyET0(
    ...
    pressure = pressureFromAltitude(height) / 1000.0
    # saturation vapor pressure [kPa]
    satVapPressure = saturationVaporPressure(airTemperature) / 1000.0
    # current vapor pressure [kPa]
    vaporPressure = satVapPressure * (airRelHumidity / 100.0)
    ...
    netLWRadiation = (
        cloudFactor * emissivity * STEFAN_BOLTZMANN * math.pow(airTempKelvin, 4.0)
    )
    # net radiation [J m-2 s-1]
    netRadiation = (1.0 - ALBEDO_CROP_REFERENCE) * globalSWRadiation - netLWRadiation

    # from [W m-2] to [J m-2 h-1]
    netRadiation = netRadiation * 3600.0
    ...
    slope = 4098.0 * satVapPressure / (airTempKelvin * airTempKelvin)
    # latent heat of vaporization [J kg-1]
    latentHeatVap = 2501000.0 - 2369.2 * airTemperature
    # psychrometric instrument constant [kPa K-1]
    psychro = CP * pressure / (RATIO_WATER_VD * latentHeatVap)

    denominator = slope + psychro * (1.0 + Cd * windSpeed_2m)
    firstTerm = slope * (netRadiation - g) / (latentHeatVap * denominator)
    secondTerm = (
        psychro
        * (37.0 / airTempKelvin)
        * windSpeed_2m
        * (satVapPressure - vaporPressure)
    ) / denominator

    return max(firstTerm + secondTerm, 0.0)
